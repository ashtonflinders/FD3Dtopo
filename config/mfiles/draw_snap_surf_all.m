...
for n=1:nsnap
    ...
    [x{n},y{n},z{n}]=gather_coord(snapinfo{n},'coorddir',dir_coord);
    nx{n}=size(x{n},1);ny{n}=size(x{n},2);nz{n}=size(x{n},3);
...
for n=1:nsnap
    ...
    if flag_emlast
       sid{n}=surf(squeeze(permute(x{n},[2 1 3])), ...
                squeeze(permute(y{n},[2 1 3])), ...
